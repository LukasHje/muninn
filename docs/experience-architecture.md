...
Homelab Cards and the Homelab Inspector consume the same category-artwork resolver. The Inspector intentionally ignores note-owned covers, thumbnails, and body images just as the cards do. Its contextual toolbar is followed by a compact full-width artwork hero and one continuous editorial content surface. The inspector panel, shell, scroll remainder, and content use one shared Homelab surface token so short previews never expose the generic drawer background. Identity, purpose, comparable facts, extended details, and relationships are separated by typography and dividers rather than nested bordered panels. Comparable card facts lead the technical hierarchy; duplicate fields are omitted from the extended details list. Purpose text is read from normalized `purpose`, `usecase`, or `description` metadata so raw inline Dataview source such as `= this.usecase` never appears in Application UI.
