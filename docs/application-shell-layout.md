...
Favorites navigation counts resolved vault notes, not the raw number of IDs in
Experience State. IDs left behind by renamed, moved, or deleted notes remain
non-destructively stored but do not inflate the sidebar count or toggle response.
...
Below `768px`, the fixed Experience discovery chrome initializes expanded and may compress into a compact 70–100 pixel Hero. `ExperienceView` owns this behavior for every Experience and includes the hero, statistics, filters, sorting/layout controls, and optional dashboard in the compressible region. Compact mode retains cropped registered artwork, a dark readability gradient, Experience title, summary statistics, and a full-surface expansion target. It is the same Hero presentation changing size, not a second accordion component. Collapsing only releases vertical space to the existing note browser; it must not change URL state, filter state, result state, or scroll ownership. Wider viewports always show the complete discovery chrome.
