...
test("operating system aliases resolve to their canonical definitions", () => {
	...
	assert.equal(resolveOperatingSystem("CachyOS (x86_64)").id, "cachyos");
	assert.equal(resolveOperatingSystem("cachyos-x86-64").id, "cachyos");
});
...
test("operating system formatting accepts separate and inline release information", () => {
	...
	assert.equal(formatOperatingSystem("CachyOS (x86_64)").displayLabel, "CachyOS");
	assert.equal(formatOperatingSystem("CachyOS x86-64 optimized desktop").displayLabel, "CachyOS");
	assert.equal(formatOperatingSystem("CachyOS (x86_64)", "2026.07").displayLabel, "CachyOS 2026.07");
});
...
test("operating system fallbacks distinguish generic Linux from unknown systems", () => {
	...
	assert.equal(operatingSystems.length, 16);
